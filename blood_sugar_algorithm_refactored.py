...
from exceptions import PropertyUnavailable, InvalidArgument

SPACE = " "
VALID_STATES = ["RBS", "FBS", "PBS", "PLS", "PDS"]

def random_datetime() -> datetime:
    return datetime.today() - timedelta(minutes=random.randint(5, 3600))

class BloodSugar():
    def __init__(self, bs: int, temperature: int, m_state: str, m_time: datetime):
        if bs > 500 or bs < 0:
            raise InvalidArgument(property=_("bs_type"))
        if temperature < -30 or temperature > 60:
            raise InvalidArgument(property=_("bs_type"))
        if m_state not in VALID_STATES:
            raise InvalidArgument(property=_("bs_type"))
        self.blood_sugar = bs
...
class BloodSugarAlgorithm(abstract_algorithm.Algorithm):
    ...
    def set_prev_bs_info(self, prev_bs: int, prev_mtime: datetime, prev_bs_typ: str) -> bool:
        self.prev_bs = BloodSugar(prev_bs, 32, prev_bs_typ, prev_mtime)
        return True

    def set_patient_medical_record(self, diab_typ1: bool, diab_typ2: bool) -> bool:
        if diab_typ1:
    ...
    def check_prev_bs(self, last_bs: BloodSugar ) -> str:
        scenario = BS_Scenario_Ids.SCBSN000
        if last_bs.blood_sugar < 60:
    ...
    def check_super_high_bs(self, last_bs: BloodSugar) -> str:
        scenario = BS_Scenario_Ids.SCBSN000
        if last_bs.blood_sugar >= 250:
    ...
    def diabetic_persons_bs(self, last_bs: BloodSugar) -> str:
        scenario = BS_Scenario_Ids.SCBSN000
        if self.diabetes_type_2 or self.diabetes_type_1:
            if self.bs_type == "RBS":
                if last_bs.blood_sugar >= 200:
                ...
                if last_bs.blood_sugar > 130:
                    scenario = BS_Scenario_Ids.SCBSN013
                    self.abnormal_bs = True
                    self.graphic_asset = POKER_SINACARE
                    message = self.__messages[scenario]
                elif 80 <= last_bs.blood_sugar <= 130:
                    scenario = BS_Scenario_Ids.SCBSN014
                    self.graphic_asset = EXCELLENT_SINACARE
                    message = self.__messages[scenario]
                elif 70 <= last_bs.blood_sugar < 80:
    ...
    def normal_person_faten_bs(self, last_bs: BloodSugar) -> str:
        if last_bs.blood_sugar >= 125:
            scenario = BS_Scenario_Ids.SCBSN006
            self.abnormal_bs = True
            self.graphic_asset = POKER_SINACARE
            message = self.__messages[scenario]
        elif 100 <= last_bs.blood_sugar < 125:
    ...
    def normal_person_bs(self, last_bs: BloodSugar) -> str:
        scenario = BS_Scenario_Ids.SCBSN000
        if self.bs_type == "RBS":
            if last_bs.blood_sugar >= 200:
            ...
            scenario = self.normal_person_faten_bs(last_bs)
        elif self.bs_type in ["PBS", "PLS", "PDS"]:
            if last_bs.blood_sugar >= 200:
    ...
    def run(self, current_bs: BloodSugar) -> str:
        scenario_str = check_prev_bs(current_bs)
        if scenario_str != BS_Scenario_Ids.SCBSN000.name:
            return scenario_str
        scenario_str = check_super_high_bs(current_bs)
        if scenario_str != BS_Scenario_Ids.SCBSN000.name:
            return scenario_str
        scenario_str = diabetic_persons_bs(current_bs)
        if scenario_str != BS_Scenario_Ids.SCBSN000.name:
            return scenario_str
        scenario_str = normal_person_bs(current_bs)
        if scenario_str != BS_Scenario_Ids.SCBSN000.name:
            return scenario_str
        else:
            raise Exception()
